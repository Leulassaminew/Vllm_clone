...
class vLLMEngine:
    ...
    async def generate_vllm(self, llm_input, validated_sampling_params, batch_size, stream, apply_chat_template, personality, background, character, context, conv, request_id: str) -> AsyncGenerator[dict, None]:

        past = """You are going to impersonate a person named {character}
            This is his personality {personality}.\n
            This is his background {background}.\n
            Always impersonate {character} never reveal youre an AI and always use a talking style that will fit {character}'s background
            Remember this conversation is via text message
            If the context below relates to the question use it to answer the question:\n
            Context {context}
            conversation\n
        """
        past=past.replace("{character}",character)
        past=past.replace("{personality}",personality)
        past=past.replace("{background}",background)
        past=past.replace("{context}",context)
        llm_input = past + conv + "user:" + llm_input + character + ":"
